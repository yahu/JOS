...
spawn(const char *prog, const char **argv)
{
	...
	if (readn(fd, elf_buf, sizeof(elf_buf)) != sizeof(elf_buf)
		...
		close(fd);
		//cprintf("[%x] spawn 2,fd = %d\n",sys_getenvid(),fd);
		//cprintf("fd%x 's offset is %d\n",fd,file_offset(fd));
...
copy_shared_pages(envid_t child)
{
	...
	for(addr = 0;addr < ULIM;addr += PGSIZE){
	    if(!(vpd[PDX(addr)] & PTE_P))
            continue;
	    if((vpt[PGNUM(addr)] & PTE_P) && (vpt[PGNUM(addr)]&PTE_SHARE)){
	        if(( r = sys_page_map(0, (void*)addr, child, (void*)addr,PGOFF(vpt[PGNUM(addr)]) )) < 0)
                return r;
	    }
	}
	return 0;
