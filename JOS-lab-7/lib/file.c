...
open(const char *path, int mode)
{
   ...
   // cprintf("UTOP - (uint32_t)path = %x", UTOP - (uint32_t)path);
    memmove(fsipcbuf.open.req_path,path,strlen(path));//strlen(path)
    ...
    //cprintf("in open ,[%x,%s],&fsipcbuf:%x,vpt[PGNUM(&fsipcbuf)]:%x ,path =%s,fd = %d\n",
	//			sys_getenvid(), binaryname, (uint32_t)(&fsipcbuf),vpt[PGNUM(&fsipcbuf)],fsipcbuf.open.req_path,fdidx);
...
devfile_read(struct Fd *fd, void *buf, size_t n)
{
	...
	//struct Fsreq_read* req = (struct Fsreq_read*)buf;
	int r;
