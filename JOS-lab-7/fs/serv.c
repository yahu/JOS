...
serve_init(void)
{
	...
	//sys_page_alloc(0,(void *)fsreq,PTE_P | PTE_U |PTE_W);
}
...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    if(n > PGSIZE)
        n = PGSIZE;
    if((n = file_read(of->o_file, ret->ret_buf, n, of->o_fd->fd_offset)) < 0)
        return n;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	struct OpenFile *of;
    int n;
    if((n=openfile_lookup(envid, req->req_fileid, &of)) < 0)
        return n;
    if((n=file_write(of->o_file, req->req_buf, req->req_n, of->o_fd->fd_offset)) < 0)
        return n;
    of->o_fd->fd_offset += n;
...
serve(void)
{
	...
	while (1) {
		...
		if (debug)
			cprintf("fs req %d from %08x [page %08x: %x]\n",
		...
		if (req == FSREQ_OPEN) {
		    if(debug)
                cprintf("in serve,req == FSREQ_OPEN,path:%s\n",((struct Fsreq_open*)fsreq)->req_path);
