...
page_alloc(int alloc_flags)
{
    ...
    if(page2pa(pg) == 0)
        panic("\n\nin pmap.c/page_alloc,page2pa(pg) == 0 is been allocted\n\n");
...
page_free(struct Page *pp)
{
    ...
    if(pp->pp_ref)
        return;
    if(page2pa(pp) == 0)
        panic("in page_free,phyaddr 0 is been add to page_free_list\n");
...
{
    if(page2pa(pp) == 0)
        cprintf("in decref,phyaddr 0 's ref = %d\n",pp->pp_ref);
    if (--pp->pp_ref == 0)
        page_free(pp);
...
page_remove(pde_t *pgdir, void *va)
{
   ...
   // cprintf("page_remove test1,pte= %x\n",pte);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    vstart = ((uint32_t) va) & (~0xfff);
    vend = ((uint32_t)va + len - 1) & (~0xfff);

    if(vend >= ULIM){
        if(vstart < ULIM)
            user_mem_check_addr = ULIM;
        else
            user_mem_check_addr = vstart;
        return -E_FAULT;

    }

    for(i=vstart;i<=vend;i+=PGSIZE){
        ptep = pgdir_walk(env->env_pgdir,(void *)i,0);

        if(!ptep || ( (*ptep) & (perm  |PTE_P) ) != (perm | PTE_P) ){ //| PTE_U

            cprintf("mem check find error \n");
            if(!ptep)
                cprintf("!ptep\n");
            else{
                if(( (*ptep) & (perm  |PTE_P) ) != (perm |PTE_P))
                    cprintf("*ptep =%x,( (*ptep) & (perm  |PTE_P) ) != (perm  |PTE_P)\n"
                            ,(uint32_t)*ptep);
            }
