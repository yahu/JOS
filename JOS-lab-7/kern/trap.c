...
trap_init(void)
{
    ...
    extern char divide_error[];
    extern char debug[];
    extern char nmi[];
    extern char break_point[];
    extern char overflow[];
    extern char bounds[];
    extern char invalid_op[];
    extern char device_not_available[];
    extern char double_fault[];
    ...
    extern char invalid_TSS[];
    extern char segment_not_present[];
    extern char stack_segment[];
    extern char general_protection[];
    ...
    extern char float_point_error[];
    extern char alignment_check[];
    extern char machine_check[];
    extern char SIMD_float_point_error[];
  ...
  //  SETGATE(idt[T_SYSCALL], 0, GD_KT, system_call, 3);
    ...
    extern char irq_timer();
    extern char irq_kbd();
    extern char irq_serial();
    extern char irq_spurious();
    extern char irq_ide();
    extern char irq_error();
...
trap_init_percpu(void)
{
	...
	uint32_t kstacktop =  KSTACKTOP - cpu_num * (KSTKSIZE + KSTKGAP);

    cpus[cpu_num].cpu_ts.ts_esp0 = kstacktop;
    cpus[cpu_num].cpu_ts.ts_ss0 = GD_KD;

    gdt[(GD_TSS0 >> 3) + cpu_num] = SEG16(STS_T32A, (uint32_t) (&cpus[cpu_num].cpu_ts),
...
trap_dispatch(struct Trapframe *tf)
{
	...
	if (tf->tf_trapno == IRQ_OFFSET + IRQ_SPURIOUS) {
		...
		print_trapframe(tf);
		//panic("Spurious interrupt on irq 7");
		return;
	}
...
trap(struct Trapframe *tf)
{
	...
	//cprintf("trap_dispatch(tf) finish\n");

	if (curenv && curenv->env_status == ENV_RUNNING){
	   // cprintf("duppage 1.1.1\n");
	    env_run(curenv);
	}

	else{
	    cprintf(" in trap end, sched_yield();\n");
	    sched_yield();
	}
		//sched_yield();
}
...
*/
	...
	if(!curenv->env_pgfault_upcall){
	    cprintf("current env's env_pgfault_upcall has't registered\n");
	    cprintf("[%08x] user fault va %08x ip %08x\n",
		curenv->env_id, fault_va, tf->tf_eip);
        print_trapframe(tf);
	    env_destroy(curenv);
   ...
   // cprintf("in page_fault_handler,utf->utf_eflags 's IF=%x\n",(utf->utf_eflags&FL_IF) == 0? 0:1);
    //if page fault happen in kernel,the user_mem_assert will find it and immediatly
    //exit the process,so we can assume cs is GD_UT
