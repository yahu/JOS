...
static int num_flist()
{

    int nfree=0;
    struct Page* pg=page_free_list;
    while(pg)
    {
        nfree++;
        pg=pg->pp_link;
...
boot_alloc(uint32_t n)
{
    ...
    /*
    if(n==0)
        cprintf("the free pages in initial 4MB is %d\n",
                 ( (int)(KERNBASE+0X400000)-(int)nextfree) / PGSIZE );
    */
    ...
    //check whether end is out of the boundary just because only the vitual space
    // [KERNBASE,KERNBASE+0X400000] has been mapped

    if(! nextfree%PGSIZE )
        panic("nextfree must be aligned o a multiple of PGSIZE\n");

    //result is virtual address , using virtual address like this is right ?
    result = nextfree;

    n+=PGSIZE;
    while(n>PGSIZE)
    {
        if( (unsigned)(nextfree = nextfree + PGSIZE) >= KERNBASE+0X400000)
            panic("Only 4MB  sapce has been mapped \n");
        n-=PGSIZE;
    }

    //cprintf("nextfree=%x,result=%x\n",nextfree,result);
    return result;
}
...
/*
    // debug info
    cprintf("in boot_phys_map=%x\n",(int)entry_pgdir);
    cprintf("entry_pgdir[PDX(0)]=%x\n",(int)entry_pgdir[PDX(0)]);
    cprintf("entry_pgdir[PDX(0xf0000000)]=%x\n",(int)entry_pgdir[PDX(0xf0000000)]);
...
/*
    // debug info
    cprintf("npages*PGSIZE/1024=%dK",npages*PGSIZE/1024);
    cprintf("npages=%d\n",npages);
    cprintf("actual_top=%x \n",actual_top );
    cprintf("npts=%d\n",npts);
    boot_alloc(0);
*/
    ...
    for(i=0; i<npts && sp<actual_top; i++){
      ...
      //   cprintf("sp=%x,(sp-KERNBASE)/PGSIZE=%dK\n",sp,(sp-KERNBASE)/PGSIZE);
    }
/*
    int ii;
    for(ii=PDX(KERNBASE);ii<1024;ii++){
        cprintf("the entry_pgdir'%dth pde is  %x\n ",ii,(int)entry_pgdir[ii]);
    }

    uintptr_t ts_addr=0xf00c0000;
    physaddr_t ts_paddr=( (pte_t *)(PTE_ADDR( entry_pgdir[PDX(ts_addr)] ) + KERNBASE) )[PTX(ts_addr)];
    cprintf("ts_addr=%x,ts_paddr=%x\n ",ts_addr,ts_paddr);
      */
...
addr_info(uintptr_t va)
{
    ...
    //cprintf("aentry_pgdir v:%x p:%x \n",entry_pgdir,(int)entry_pgdir-(int)KERNBASE);
    //cprintf("temppde=(pde_t) *((uint32_t*)entry_pgdir+1023) : %x\n",temppde);
}
...
*/
    ...
    // each physical page, there is a corresponding struct Page in this
    // array.  'npages' is the number of physical pages in memory.
    // Your code goes here:(struct page *)
    pages=(struct Page*)boot_alloc(sizeof(struct Page)*npages);


    //addr_info((uint32_t)temppde);*/
    //addr_info(0xf1000000);
    ...
    page_init();
/*
    struct Page *tspg=page_alloc(0);
    page_insert(kern_pgdir,tspg,(void*)PTSIZE,PTE_P);
    cprintf("struct Page* %x in first insert , ref is %x\n",(int)tspg,tspg->pp_ref);
    page_insert(kern_pgdir,tspg,(void*)PTSIZE,PTE_P | PTE_U);
    cprintf("struct Page* %x in second insert, ref is %x\n",(int)tspg,tspg->pp_ref);
    panic("asdf\n");
*/
    check_page_free_list(1);
    check_page_alloc();
    ...
    boot_map_region(kern_pgdir, UPAGES, ROUNDUP(npages*sizeof(struct Page) ,PGSIZE),
                   PADDR(pages), PTE_P | PTE_W);

    // panic("stop map UPAGES\n");
    //////////////////////////////////////////////////////////////////////
    // Use the physical memory that 'bootstack' refers to as the kernel
    // stack.  The kernel stack grows down from virtual address KSTACKTOP.
    // We consider the entire range from [KSTACKTOP-PTSIZE, KSTACKTOP)
    ...
    boot_map_region(kern_pgdir, KERNBASE,0x10000000,
                    0, PTE_P | PTE_W);
...
//sub page_init funciton ,to add pages[sidx] to pages[eidx] into page_free_list
//only called by page_init.
void
_page_init(uint32_t sidx,uint32_t eidx )
{
...
page_alloc(int alloc_flags)
{
    ...
    //cprintf("page_alloc gets one page whose phiscal address is %x,kernal virtua address is %x\n ",
    //  page2pa(pg),page2kva(pg) );

    //*page2kva(pg)
    //addr_info((uint32_t)page2kva(pg));
...
page_decref(struct Page* pp)
{
    if (--pp->pp_ref == 0)
        page_free(pp);
  //  cprintf("struct Page* %x after page_decref, ref is %x\n",(int)pp,pp->pp_ref);
}
...
/*
    if(pa2page(PTE_ADDR(*pte))==pp){
        pp->pp_ref++;
        // cprintf("pa2page(PTE_ADDR(*pte))==pp\n");
    }
*/
    pp->pp_ref++;
...
//=======
    ...
    for (i = 0; i < npages * PGSIZE; i += PGSIZE){
        /*
        cprintf("check_va2pa(pgdir, KERNBASE + %x)=%x\n",
                i,check_va2pa(pgdir, UPAGES + i));
        cprintf("i=%x\n",i);
        */
