...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
   ...
   // if(dstenv->env_ipc_from !=0 || dstenv->env_ipc_recving == 0){ //dstenv->env_status != ENV_NOT_RUNNABLE
      //  cprintf("[%x],dstenv %x env_status is %x,and env_ipc_recving is %x\n",
       //        sys_getenvid() ,dstenv->env_id,dstenv->env_status,dstenv->env_ipc_recving);
        return -E_IPC_NOT_RECV;
    }

    //cprintf("in middle ipc_try_send\n");
    if( (uint32_t)srcva < UTOP ){
        ...
        if(perm & PTE_W)
            checkperm |=PTE_W;
        if(user_mem_check(curenv, srcva, PGSIZE, PTE_P) < 0)
            return -E_INVAL;
   ...
   // cprintf("[%x],dstenv %x 's env_ipc_from is %x\n",
    //        sys_getenvid(),dstenv->env_id,dstenv->env_ipc_from);
...
sys_ipc_recv(void *dstva)
{
	...
	if((uint32_t)dstva < UTOP && ( (uint32_t)dstva & 0xfff ) !=0){
        //cprintf("%x ,sys_ipc_recv,return -E_INVAL\n",sys_getenvid());
	     return -E_INVAL;
	}
   ...
   // cprintf("sys_ipc_recv before sched_yield,[%x] 's status is %x\n ",curenv->env_id,curenv->env_status);
    sched_yield();
